...
- Place recipe definition classes in the existing craft-skill folder pattern and name them for the recipe family, set, product line, or concrete item they define. Group related recipes into a coherent family file when they share a reward category, product line, or use pattern, such as provision recipes or field-tool recipes. Use a one-recipe class only when the recipe is genuinely standalone and no sensible family exists. Do not lump unrelated recipes into planet/rarity aggregate classes such as `ViscaraRareEliteSmitheryRecipes`. Production recipe declarations must use explicit fluent `_builder.Create(...).Category(...).Resref(...)` blocks like the existing recipe files; do not compress recipe entries into helper calls, tuple arrays, generic entry records, or other packed table formats.
- When a named rare elite adds multiple recipes in the same item family, vary `.Level(...)` values across the intended area or reward band instead of assigning every item the same level or clustering the set around only one or two repeated numbers. Keep the matching Design Bible recipe row levels and derived tier columns synchronized with the code.
...
- Fabrication furniture recipe outputs must be registered with the property structure system. Use `structure_####` crafted output resrefs, add matching active `StructureType` entries with valid placeable resrefs, and make dropped blueprint resrefs point at those structure outputs. Tests should verify each furniture recipe output maps to an active `StructureType`.
