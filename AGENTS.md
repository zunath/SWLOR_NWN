...
- Items equipped on NPCs (`utc` `Equip_ItemList`) that are not obtainable by players through any source must carry the `NO_ECONOMY` flag. `EconomyEquipCoverageTests` enforces this: it derives the set from creature equip lists minus every obtainable source (loot, stores, treasure containers, recipe outputs/components, refining, fishing, quest rewards, training store, starting gear, and `CreateItemOnObject` literals). When it fails on a newly added NPC item, run `python tools/FlagNpcEconomyItems.py` to stamp it (or make the item obtainable). New flags require a module repack on deploy.
