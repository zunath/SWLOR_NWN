    ...
    public static unsafe class OnAIActionAttackObject
    {
        ...
        private static int HandleOnAIActionAttackObject(void* creature, void* node)
        {
            return _executor.ExecuteInScriptContext(() =>
            {
                ...
                {
                    var calculatedDelay = _statService.CalculateAttackDelay(pCreature.m_idSelf);
                                ...
                                case CNWSCOMBATROUND_TYPE_ATTACK:
                                    {
                                        ...
                                        if (!bOverrideAction)
                                        {
                                            ...
                                            if (isParalyzed)
                                            {
                                                _logger.Write<AttackLogGroup>($"Creature {pCreature.m_idSelf:X8} is paralyzed, recomputing round");
