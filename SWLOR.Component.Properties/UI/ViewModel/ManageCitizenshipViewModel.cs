...
namespace SWLOR.Component.Properties.UI.ViewModel
{
    public class ManageCitizenshipViewModel: GuiViewModelBase<ManageCitizenshipViewModel, IGuiPayload>
    {
        ...
        public ManageCitizenshipViewModel(IGuiService guiService, ILogger logger, IWorldPropertyRepository worldPropertyRepository, IPlayerRepository playerRepository, IElectionRepository electionRepository, IWorldPropertyPermissionRepository worldPropertyPermissionRepository, IServiceProvider serviceProvider) : base(guiService)
        {
            _logger = logger;
        ...
        private void LoadData()
        {
            ...
            var dbCity = _worldPropertyRepository.GetById(dbBuilding.ParentPropertyId);
            var dbMayorPlayer = _playerRepository.GetById(dbCity.OwnerPlayerId);
            var dbElection = _electionRepository.GetSingleByPropertyId(dbCity.Id);
            var dbCitizenCount = _playerRepository.GetByCitizenPropertyId(dbCity.Id)
                .Count(p => !p.IsDeleted);
        ...
        public Action RegisterRevoke() => () =>
        {
            ...
            if (dbPlayer.CitizenPropertyId == _cityPropertyId)
            {
                ShowModal("Revoking your citizenship will remove your ability to access all public facilities. You will also lose any permissions granted to you. If you are a candidate for an election, you will be withdrawn from the race. Are you sure you want to revoke your citizenship?",
                    () =>
                    {
                        var dbCity = _worldPropertyRepository.GetById(_cityPropertyId);
                        ...
                        var structures = _worldPropertyRepository.GetByPropertyIds(structureIds);
                        ...
                        var permissions = _worldPropertyPermissionRepository.GetByPlayerId(playerId)
                            .Where(p => interiorPropertyIds.Contains(p.PropertyId));
        ...
        {
            _guiService.TogglePlayerWindow(Player, GuiWindowType.Election, null, TetherObject);
        };
    }
}
