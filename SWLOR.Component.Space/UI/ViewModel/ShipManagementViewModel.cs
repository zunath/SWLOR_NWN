...
namespace SWLOR.Component.Space.UI.ViewModel
{
    public class ShipManagementViewModel : GuiViewModelBase<ShipManagementViewModel, ShipManagementPayload>
    {
        ...
        private int CalculateRepairBill(PlayerShip ship)
        {
            ...
            var starportBonus = PropertyService.GetEffectiveUpgradeLevel(dbPlayer.CitizenPropertyId, PropertyUpgradeType.StarportLevel) * 0.05f;
        ...
        protected override void Initialize(ShipManagementPayload initialPayload)
        {
            ...
            if (!string.IsNullOrWhiteSpace(initialPayload.SpecificPropertyId))
            {
                ...
                var landingArea = string.IsNullOrWhiteSpace(landingPropertyLocation.AreaResref)
                    ? PropertyService.GetRegisteredInstance(landingPropertyLocation.InstancePropertyId).Area
                    : AreaService.GetAreaByResref(landingPropertyLocation.AreaResref);
            ...
            ShipCountRegistered = $"Ships: {dbPlayerShips.Count} / {SpaceService.MaxRegisteredShips}";
            LoadShip();
        ...
        private void LoadShip()
        {
            ...
            else
            {
                ...
                if (module != null)
                {
                    var detail = SpaceService.GetShipModuleDetailByItemTag(module.ItemTag);
        ...
        private uint GetShipLocation(WorldProperty property)
        {
            ...
            else
            {
                ...
                var area = string.IsNullOrWhiteSpace(landingLocation.AreaResref)
                    ? PropertyService.GetRegisteredInstance(landingLocation.InstancePropertyId).Area
                    : AreaService.GetAreaByResref(landingLocation.AreaResref);
        ...
        {
            IsRegisterEnabled = _shipIds.Count < SpaceService.MaxRegisteredShips && IsMyShipsToggled;
        ...
        {
            TargetingService.EnterTargetingMode(Player, ObjectType.Item, "Please click on a ship deed within your inventory.",
                item =>
            {
                ...
                if (!ItemService.CanCreatureUseItem(Player, item))
                {
                ...
                {
                    FloatingTextStringOnCreature($"You may only have {SpaceService.MaxRegisteredShips} ships registered at a time.", Player, false);
                    return;
                }

                // Validation passed. Add the ship and register it under this player.
                var itemTag = GetTag(item);
                var shipDetail = SpaceService.GetShipDetailByItemTag(itemTag);
                var bonuses = SpaceService.GetShipBonuses(item);

                // Spawn the property associated with this ship.
                var property = PropertyService.CreateStarship(
                    Player, 
                    shipDetail.Layout, 
                    _planetType,
                    _spaceLocation, 
                ...
                ShipCountRegistered = $"Ships: {dbPlayerShips.Count + 1} / {SpaceService.MaxRegisteredShips}";
        ...
        public Action OnClickUnregisterShip() => () =>
        {
            ShowModal($"Unregistering a ship will convert it back into a deed item. Any structures contained inside will be permanently lost. Are you sure you wish to unregister this ship?",
                () =>
                {
                    ...
                    SelectedShipIndex = -1;
                    ToggleRegisterButtons();
                    ShipCountRegistered = $"Ships: {_shipIds.Count} / {SpaceService.MaxRegisteredShips}";
                    LoadShip();
        ...
        private bool ValidateModuleEquip(PlayerShip dbShip, uint item)
        {
            ...
            var moduleDetails = SpaceService.GetShipModuleDetailByItemTag(itemTag);
            var shipDetails = SpaceService.GetShipDetailByItemTag(dbShip.Status.ItemTag);
            ...
            if (!SpaceService.CanPlayerUseShipModule(Player, itemTag))
            {
        ...
        private void ProcessHighPower(int slot)
        {
            ...
            {
                var moduleDetail = SpaceService.GetShipModuleDetailByItemTag(module.ItemTag);
