...
using SWLOR.Core.Feature.GuiDefinition.RefreshEvent;
...
using SWLOR.Core.Service.FactionService;
using SWLOR.Core.Service.GuiService;
using Faction = SWLOR.Core.Service.Faction;
using ChatChannel = SWLOR.Core.NWNX.Enum.ChatChannel;
    ...
    public class DMChatCommand: IChatCommandListDefinition
    {
        ...
        private void GetLocalVariable()
        {
            _builder.Create("getlocalfloat")
                ...
                .Action((user, target, location, args) =>
                {
                    ...
                    var value = NWScript.NWScript.GetLocalFloat(target, variableName);
        ...
        private void GetTag()
        {
            _builder.Create("gettag")
                ...
                .Action((user, target, location, args) =>
                {
                    var tag = NWScript.NWScript.GetTag(target);
