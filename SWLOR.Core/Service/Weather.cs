...
using SWLOR.Core.NWScript.Enum.Area;
using SWLOR.Core.NWScript.Enum.VisualEffect;
using SWLOR.Core.Service.WeatherService;
    ...
    public static class Weather
    {
        ...
        public static void SetWeather(uint oArea)
        {
            ...
            if (nHumidity > 7 && nHeat > 3)
            {
                if (nHeat < 6 && nWind < 3)
                {
                    NWScript.NWScript.SetWeather(oArea, WeatherType.Clear);
                }
                else NWScript.NWScript.SetWeather(oArea, WeatherType.Rain);
            }
            else if (nHumidity > 7) NWScript.NWScript.SetWeather(oArea, WeatherType.Snow);
            else NWScript.NWScript.SetWeather(oArea, WeatherType.Clear);
        ...
        public static SWLOR.Core.NWScript.Enum.Weather GetWeather()
        {
            return GetWeather(OBJECT_SELF);
        }

        public static SWLOR.Core.NWScript.Enum.Weather GetWeather(uint oArea)
        {
            if (GetIsAreaInterior(oArea) || GetIsAreaAboveGround(oArea) == false)
            {
                return SWLOR.Core.NWScript.Enum.Weather.Invalid;
            }
            ...
            {
                return SWLOR.Core.NWScript.Enum.Weather.Foggy;
            }

            // Rather unfortunately, the default method is also called GetWeather. 
            return NWScript.NWScript.GetWeather(oArea);
        }
        ...
        public static void DoWeatherEffects(uint oCreature)
        {
            ...
            // Arrow spell.
            //--------------------------------------------------------------------------
            if (bIsPC && NWScript.NWScript.GetWeather(oArea) == SWLOR.Core.NWScript.Enum.Weather.Rain && GetLocalInt(oArea, VAR_WEATHER_ACID_RAIN) == 1)
            {
                var eEffect =
                  EffectLinkEffects(
                      EffectVisualEffect(VisualEffect.Vfx_Imp_Acid_S),
        ...
        public static void OnAreaEnter()
        {
            ...
            if (nHour != nLastHour)
            {
                ...
                if (nWeather == SWLOR.Core.NWScript.Enum.Weather.Foggy)
                {
