...
public class AbilityImpactAnimationAuditTests
{
    ...
    public void ActivationAnimationOverwrite_UsesExplicitThrowPreservingPlayback()
    {
        ...
        var helperCall = System.Text.RegularExpressions.Regex.Match(
            ...
            @"PlayAnimationWithTemporaryReplacementPreservingExplicitThrow\s*\(\s*activator,\s*AbilityAnimationBinding\.ActivationType\(ability,\s*activator,\s*animationLength\),\s*1\.0f,\s*animationLength,\s*sourceAnimationName,\s*replacementAnimationName,\s*ability\.AnimationRestoreDelaySeconds\s*\)",
            System.Text.RegularExpressions.RegexOptions.CultureInvariant);
