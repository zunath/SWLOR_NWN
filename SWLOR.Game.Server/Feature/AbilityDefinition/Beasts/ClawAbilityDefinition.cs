...
namespace SWLOR.Game.Server.Feature.AbilityDefinition.Beasts
{
    public class ClawAbilityDefinition : IAbilityListDefinition
    {
        ...
        private void ImpactAction(uint activator, uint target, int dmg, int dc)
        {
            ...
            {
                StatusEffect.Apply(activator, target, StatusEffectType.Bleed, 30f);
            }
        ...
        private void Claw1()
        {
            _builder.Create(FeatType.Claw1, PerkType.Claw)
                ...
                {
                    ImpactAction(activator, target, 8, 8);
                });
        }
        private void Claw2()
        {
            _builder.Create(FeatType.Claw2, PerkType.Claw)
                ...
                .HasImpactAction((activator, target, level, location) =>
                {
                    ImpactAction(activator, target, 11, 10);
                });
        }
        private void Claw3()
        {
        ...
        private void Claw4()
        {
            _builder.Create(FeatType.Claw4, PerkType.Claw)
                ...
                .HasImpactAction((activator, target, level, location) =>
                {
                    ImpactAction(activator, target, 17, 14);
                });
        }
        private void Claw5()
        {
            _builder.Create(FeatType.Claw5, PerkType.Claw)
                ...
                .HasImpactAction((activator, target, level, location) =>
                {
                    ImpactAction(activator, target, 20, 16);
                });
        }
