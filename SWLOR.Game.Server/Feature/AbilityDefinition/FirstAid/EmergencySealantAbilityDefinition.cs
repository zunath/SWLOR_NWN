...
{
    public sealed class EmergencySealantAbilityDefinition : IAbilityListDefinition
    {
        public Dictionary<FeatType, AbilityDetail> BuildAbilities()
        {
            var builder = new AbilityBuilder();

            EmergencySealant1(builder);

            return builder.Build();
        }

        private static void EmergencySealant1(AbilityBuilder builder)
        {
            builder
                .Create(FeatType.EmergencySealant1, PerkType.EmergencySealant)
                .Name("Emergency Sealant I")
                .Level(1)
                .HasActivationDelay(1f)
                .HasRecastDelay(RecastGroup.EmergencySealant, 18f)
                ...
                .HasImpactAction(EmergencySealant1ImpactAction)
                .IsCastedAbility()
                .BreaksStealth()
        ...
        private static void EmergencySealant1ImpactAction(uint activator, uint target, int level, Location targetLocation)
        {
            foreach (var friendly in AbilityTargeting.GetFriendlyTargets(activator, target, false))
            {
                StatusEffect.RemoveFirstStatusEffect(friendly, new[] { typeof(BleedStatusEffect), typeof(BurnStatusEffect) }, false);
                StatusEffect.ApplyStatusEffect(
                    activator,
                    friendly,
                    new RegenerativeHealingStatusEffect("Emergency Sealant", 8f, 4),
                    12f);
