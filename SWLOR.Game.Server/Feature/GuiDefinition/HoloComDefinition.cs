...
namespace SWLOR.Game.Server.Feature.GuiDefinition
{
    public class HoloComDefinition: IGuiWindowDefinition
    {
        ...
        // ------------------------------------------------------------------
        // TEMP DIAGNOSTIC - HoloCom NUI layout investigation. Each partial below
        // isolates one construct from the Messages tab. Remove after diagnosis.
        // ------------------------------------------------------------------

        /// <summary>
        /// T1: fixed-height button rows and a label only - no table, no list.
        /// </summary>
        private static void AddDebugBarsTab(GuiGroup<HoloComViewModel> group)
        {
            group.SetShowBorder(false);
            group.SetScrollbars(NuiScrollbars.None);
            group.AddColumn(col =>
            {
                ...
                {
                    row.AddLabel()
                        .SetText("DEBUG T1: rows and buttons only.")
                        .SetHeight(24f)
                        .SetHorizontalAlign(NuiHorizontalAlign.Left);
                });
        ...
        /// <summary>
        /// T2: text-only table with a header row, terminal - the CharacterSheet shape.
        /// </summary>
        private static void AddDebugTextTableTab(GuiGroup<HoloComViewModel> group)
        {
            group.SetShowBorder(false);
            group.SetScrollbars(NuiScrollbars.None);
            group.AddColumn(col =>
            {
                col.AddTable<HoloComViewModel>(t => t
                    .AddColumn("FROM", 180f, model => model.MessageSenderNames)
                    .AddColumn("PREVIEW", 0f, model => model.MessagePreviews)
                    .SetRowHeight(28f));
            });
        }

        /// <summary>
        /// T3: component-column table (label with color bind + button cell), no header,
        /// terminal - the MarketBuy/Contacts shape.
        /// </summary>
        private static void AddDebugComponentTableTab(GuiGroup<HoloComViewModel> group)
        {
            group.SetShowBorder(false);
            group.SetScrollbars(NuiScrollbars.None);
            group.AddColumn(col =>
            {
                col.AddTable<HoloComViewModel>(t => t
                    ...
                    {
                        cell.AddLabel()
                            .BindText(model => model.MessagePreviews)
                            .SetHorizontalAlign(NuiHorizontalAlign.Left);
                    }, isVariable: true)
                    .AddComponentColumn("", 60f, cell =>
                    {
        ...
        /// <summary>
        /// T4: the untested combination - component columns WITH a header row, terminal.
        /// </summary>
        private static void AddDebugHeaderComponentTableTab(GuiGroup<HoloComViewModel> group)
        {
            group.SetShowBorder(false);
            group.SetScrollbars(NuiScrollbars.None);
            group.AddColumn(col =>
