    ...
    public class LightsaberWorkbenchViewModel : GuiViewModelBase<LightsaberWorkbenchViewModel, GuiPayloadBase>
    {
        ...
        private void ConstructSaber()
        {
            ...
            Log.Write(LogGroup.Crafting,
                ...
                $"hasSubmissionToken={!string.IsNullOrWhiteSpace(_submissionSerialized)}", true);

            foreach (var property in _enhancementProperties.SelectMany(x => x))
            {
                // TEMPORARY DIAGNOSTIC - remove once the enhancement merge is confirmed fixed.
                Log.Write(LogGroup.Crafting,
                    $"[SABERBUILD] applying type={GetItemPropertyType(property)} value={GetItemPropertyCostTableValue(property)}", true);
