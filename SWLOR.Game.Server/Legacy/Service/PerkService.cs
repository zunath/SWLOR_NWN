...
namespace SWLOR.Game.Server.Legacy.Service
{
    public static class PerkService
    {
        ...
        private static void RegisterPerkHandlers()
        {
            ...
            {
                var instance = Activator.CreateInstance(type) as IPerkHandler;
                if (instance == null)
                {
        ...
        private static List<PCPerk> GetPCPerksByExecutionType(NWPlayer oPC, PerkExecutionType executionType)
        {
            ...
            return pcPerks.Where(x =>
            {
                ...
                var matchesExecutionType = perk.ExecutionTypeID == executionType;
                if (!matchesExecutionType) return false;

                // Filter out any perks the PC doesn't meet the requirements for.
                var effectivePerkLevel = GetPCEffectivePerkLevel(oPC, x.PerkID);
        ...
        private static void OnHitCastSpell()
        {
            ...
            var pcPerks = DataService.PCPerk.GetAllByPlayerID(oPC.GlobalID).Where(x =>
            {
                ...
                var effectiveLevel = GetPCEffectivePerkLevel(oPC, x.PerkID);
            ...
            if (type == BaseItem.SmallShield || type == BaseItem.LargeShield || type == BaseItem.TowerShield)
            {
                foreach (var pcPerk in pcPerks)
                {
                    ...
                    var spellTier = perkFeat?.PerkLevelUnlocked ?? 0;
        ...
        public static bool CanPerkBeUpgraded(NWPlayer player, int perkID)
        {
            ...
            var level = FindPerkLevel(perkLevels, rank + 1);
            ...
            {
                var pcSkill = DataService.PCSkill.GetByPlayerIDAndSkillID(dbPlayer.ID, req.SkillID);
        ...
        public static void DoPerkUpgrade(NWPlayer oPC, int perkID, bool freeUpgrade = false)
        {
            ...
            if (freeUpgrade || CanPerkBeUpgraded(oPC, perkID))
            {
                ...
                var nextPerkLevel = FindPerkLevel(perkLevels, pcPerk.PerkLevel + 1);
                ...
                if (perkFeatToGrant != null && GetHasFeat((Feat)perkFeatToGrant.FeatID, oPC.Object) == false)
                {
                    ...
                    if (Player.GetQuickBarSlot(oPC, 0).ObjectType == QuickBarSlotType.Empty)
                        Player.SetQuickBarSlot(oPC, 0, qbs);
                    else if (Player.GetQuickBarSlot(oPC, 1).ObjectType == QuickBarSlotType.Empty)
                        Player.SetQuickBarSlot(oPC, 1, qbs);
                    else if (Player.GetQuickBarSlot(oPC, 2).ObjectType == QuickBarSlotType.Empty)
                        Player.SetQuickBarSlot(oPC, 2, qbs);
                    else if (Player.GetQuickBarSlot(oPC, 3).ObjectType == QuickBarSlotType.Empty)
                        Player.SetQuickBarSlot(oPC, 3, qbs);
                    else if (Player.GetQuickBarSlot(oPC, 4).ObjectType == QuickBarSlotType.Empty)
                        Player.SetQuickBarSlot(oPC, 4, qbs);
                    else if (Player.GetQuickBarSlot(oPC, 5).ObjectType == QuickBarSlotType.Empty)
                        Player.SetQuickBarSlot(oPC, 5, qbs);
                    else if (Player.GetQuickBarSlot(oPC, 6).ObjectType == QuickBarSlotType.Empty)
                        Player.SetQuickBarSlot(oPC, 6, qbs);
                    else if (Player.GetQuickBarSlot(oPC, 7).ObjectType == QuickBarSlotType.Empty)
                        Player.SetQuickBarSlot(oPC, 7, qbs);
                    else if (Player.GetQuickBarSlot(oPC, 8).ObjectType == QuickBarSlotType.Empty)
                        Player.SetQuickBarSlot(oPC, 8, qbs);
                    else if (Player.GetQuickBarSlot(oPC, 9).ObjectType == QuickBarSlotType.Empty)
                        Player.SetQuickBarSlot(oPC, 9, qbs);
                    else if (Player.GetQuickBarSlot(oPC, 10).ObjectType == QuickBarSlotType.Empty)
        ...
        public static void CacheEffectivePerkLevel(NWPlayer player, int perkID)
        {
            ...
            var perkLevel = CalculateEffectivePerkLevel(player, perkID);
            var levels = AppCache.PlayerEffectivePerkLevels[player.GlobalID];
            ...
            using (new Profiler("PerkService::CalculateEffectivePerkLevel"))
            {
                ...
                using (new Profiler("PerkService::CalculateEffectivePerkLevel::PerkLevelIteration"))
                {
                    ...
                    foreach (var perkLevel in perkLevelsInRange)
                    {
                        var skillRequirements = DataService.PerkLevelSkillRequirement.GetAllByPerkLevelID(perkLevel.ID);
                        var questRequirements = DataService.PerkLevelQuestRequirement.GetAllByPerkLevelID(perkLevel.ID);
                        var effectiveLevel = pcPerk.PerkLevel;
