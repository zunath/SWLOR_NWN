...
namespace SWLOR.Game.Server.Perk.ForceAlter
{
    public class ThrowSaber : IPerkHandler
    {
        ...
        public void OnImpact(NWCreature player, NWObject target, int perkLevel, int spellTier)
        {
            ...
            int sabdmg = player.RightHand.DamageBonus;
            if (sabdmg > 40 ) sabdmg = 40;
            
            if (weapon.CustomItemType == CustomItemType.Lightsaber ||
                weapon.CustomItemType == CustomItemType.Saberstaff)
            {
                iDamage = sabdmg + RandomService.D6(2) + player.IntelligenceModifier + player.StrengthModifier;
            }
            else
            {
                iDamage = (int)weapon.Weight + player.StrengthModifier + (sabdmg / 2);
            }
            ...
            switch (spellTier)
            {
                case 1:
                    ...
                    player.DelayAssignCommand(() =>
                    {
                        _.ApplyEffectToObject(DurationType.Instant, _.EffectLinkEffects(_.EffectVisualEffect(VisualEffect.Vfx_Imp_Sonic), _.EffectDamage(iDamage, DamageType.BASE_WEAPON)), target);
                    }, fDelay);
                ...
                case 4:
                    ...
                    while (oObject.IsValid && iCount < 3)
                    {
                        if (oObject != target && oObject != player)
                        {
                            ...
                            player.DelayAssignCommand(() =>
                            {
                                _.ApplyEffectToObject(DurationType.Instant, _.EffectLinkEffects(_.EffectVisualEffect(VisualEffect.Vfx_Imp_Sonic), _.EffectDamage(iDamage, DamageType.BASE_WEAPON)), creature);
                            }, fDelay);
