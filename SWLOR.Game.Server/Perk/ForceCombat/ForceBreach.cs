...
namespace SWLOR.Game.Server.Perk.ForceCombat
{
    public class ForceBreach : IPerk
    {
        ...
        public void OnImpact(NWPlayer player, NWObject target, int level)
        {
            ...
            min += forceSupportBonus / 3 + wisdom / 2 + intelligence / 3;
            ...
            player.AssignCommand(() =>
            {
                Effect vfx = _.EffectVisualEffect(NWScript.VFX_IMP_DOMINATE_S);
                _.ApplyEffectToObject(NWScript.DURATION_TYPE_INSTANT, vfx, target);
            });
