...
- **CZ-220 Breaker Yard** is wired (2026-07-14). Note the area roles are reversed from the package names: **Breaker Bay (`cz220shipbreakin`) is the dungeon** and **Breaker Yard (`cz220shipbreaker`) is the boss arena**. The dungeon carries `CREATURE_SPAWN_TABLE_ID = CAPSTONE_CZ220_BREAKER_YARD`, `IS_DUNGEON`, `MINI_MAP_DISABLED`, `MAP_KEY_ITEM_ID = 23`. The boss arena has the six `quest_enc` activators (`adamg_wd_call`/`adamg_ms_call`/`scrapl_wd_call`/`scrapl_ms_call`/`wbrk_wd_call`/`wbrk_ms_call`) and their six boss spawn waypoints, all positioned at the arena's existing validated `Shipbreak_Boss_Exit` arrival point (co-located — an area builder can spread them for aesthetics; only one boss/activator is live per quest state). Covered by `CZ220BreakerYardPlacementTests`. Still to confirm by the area builder: key-gating the dungeon's external entrance with `CapstoneCZ220BreakerYardKey` (id 111) — the internal `Shipbreak_Down/Up` lifts are intentionally ungated.
