...
- Force/Devices twin lines must carry identical prices as well as identical damage and cooldowns: `Throw Rock` and `Arc Projector` (Devices 12/30/45) are both 2/3/4 SP; `Radiant Lance` and `Ion Lance` (Devices 15/32/48) are both 3/4/4 SP. Twin skill requirements may diverge per sheet (after the requirement-spread pass, `Throw Rock` sits at Force 0/18/40 and `Radiant Lance` at Force 8/35/48) because each sheet balances its own progression ladder.
- The 2026-06-11 tier-consistency pass (`tools/ApplyForceDevicesSpTierConsistency.ps1`) repriced the additive twin lines up from their placeholder 1-2 SP values and moved Force from 237 SP to 240 SP, but left Devices category totals lopsided at `58/55/56/71`. The same-day follow-up (`tools/ApplyDevicesCategoryParity.ps1`) restored category parity at `60/60/60/60`: Assault Gadgets non-twin lines were reshaped down (`Flamethrower` 2/2/3, `Wrist Rocket` 2/3/3, `Sonic Burst` 2/3/3, `Rail Dart` 2/3, `Gadget Harness I` 2, `Tactical Uplink` 2, `Cryo Sprayer I` 3), Grenadier raised `Concussion Grenade II` and `Ion Grenade II` to 4, Field Engineer raised `Blaster Beacon I` to 3, `Incendiary Field II` to 4, and `Shock Beacon II`, `Diagnostic Sweep`, and `Beacon Targeting II` to premium 5, and Field Support raised `Deflector Shield I` to 3 and `Overclock Routine`, `Rayshield Screen II`, and `Group Deflector` to premium 5. Force and Devices remain equal at 240 SP and the twin-line prices are unchanged.
...
The local Bible workbook was updated on 2026-06-08 with `tools/ApplyCombatUpgradeActivePerkBudget.ps1`, and `CombatUpgradeBiblePerkManifest.csv` was refreshed from the workbook. The pass converted 97 active-budget workbook rows to `Trait` rows and preserved stance, toggle, and aura rows as active buttons. On 2026-06-09, `tools/ApplyCombatUpgradeForceUniversal.ps1` restored the Force Universal classification and the Throw Lightsaber line. On 2026-06-11, `tools/ApplyForceDevicesAdditiveDpsBalance.ps1` restored Throw Rock as an additive Light Alter DPS line, added Radiant Lance as a new Light Sense 8m line DPS ability, and added matching Arc Projector and Ion Lance rows to Devices. On 2026-06-11, `tools/ApplyFieldSupportTraitFeedback.ps1` converted Rayshield Screen and Dampening Field into Field Support mitigation traits without changing Devices SP. On 2026-06-11, `tools/ApplyDevicesForceSpRebalance.ps1` redistributed Devices to `59/59/59/60` across its four categories and kept Force and Devices equal at 237 SP. Later on 2026-06-11, `tools/ApplyForceDevicesSpTierConsistency.ps1` repriced the placeholder twin-line prices, moving Force and Devices to an equal 240 SP, and `tools/ApplyDevicesCategoryParity.ps1` then restored equal Devices category totals at `60/60/60/60` without touching the Force sheet. `tools/RepairCombatBibleSectionTotalSp.ps1` was added to recompute the cached section Total values after price edits (run it before `tools/RepairCombatBibleTotalSp.ps1`). Later on 2026-06-11, `tools/ApplyForceRequirementSpread.ps1` rebalanced Force skill requirements across Light/Dark/Universal, gave each alignment a rank-0 entry (`Force Spark I`, `Throw Rock I`, `Benevolence I`, `Weaken Resolve I`), spread requirements evenly across the 0-50 grid, and reordered Force rows by requirement without changing any SP price. This is a design-only review; the remaining implementation cleanup should happen in a separate code pass:
...
5. Sync code perk `Price` values with the 2026-06-11 tier-consistency workbook prices and code perk skill requirements with the 2026-06-11 requirement-spread workbook values when the code pass lands; `tools/ReviewCombatBiblePerks.ps1` reports the affected rows as `PriceMismatch` and `RequirementMismatch` until then.
6. Run the focused perk review/audit scripts and the server test project before handoff.
