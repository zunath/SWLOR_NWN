...
- Force/Devices twin lines must carry identical prices and base damage: `Throw Rock` and `Arc Projector` (Devices 12/30/45) are both 2/3/4 SP; `Radiant Lance` and `Ion Lance` (Devices 15/32/48) are both 3/4/4 SP. Cooldowns may diverge when Force alignment feel requires it; `Throw Rock` uses a 6-second cooldown so Light Alter has the same spammable opener cadence as Dark Alter's `Force Spark`, while `Arc Projector` remains on the Devices 12-second direct-damage cadence. Twin skill requirements may diverge per sheet (after the requirement-spread pass, `Throw Rock` sits at Force 0/18/40 and `Radiant Lance` at Force 8/35/48) because each sheet balances its own progression ladder.
- The 2026-06-11 tier-consistency pass repriced the additive twin lines up from their placeholder 1-2 SP values and moved Force from 237 SP to 240 SP, but left Devices category totals lopsided at `58/55/56/71`. The same-day follow-up restored category parity at `60/60/60/60`: Assault Gadgets non-twin lines were reshaped down, Grenadier raised selected control rows, Field Engineer raised its strongest fields and party tools, and Field Support raised its strongest support rows. The 2026-08-13 balance correction restored `Ion Lance` to its documented 3/4/4 twin pricing and placed the four narrower core Assault Gadget lines (`Flamethrower`, `Wrist Rocket`, `Sonic Burst`, and `Rail Dart`) on a consistent 2/2/3 curve. `Gadget Harness` and `Tactical Uplink` remain 2, `Cryo Sprayer` remains 3, and `Overload Barrage` remains 5. All four Devices categories remain at 60 SP and Force and Devices remain equal at 240 SP.
- After the tier-consistency pass, Light and Dark core damage actives cost the same SP: Dark (`Force Spark` 5, `Force Lightning` 11, `Force Drain` 10, `Creeping Terror` 9) and Light (`Throw Rock` 9, `Radiant Lance` 11, `Force Judgment` 11, `Purifying Wave` 4) both total 35 SP. This Light composition is historical and was superseded on 2026-08-13 by the Force Burst and Force Judgment composition in the next entry. Dark keeps the raw damage/drain/execute identity (including the `Devouring Strike` and `Unstable Pressure` execute traits and the `Hunger of the Dark` capstone); Light keeps controlled pressure plus restraint/protection riders and the `Last Stand of the Light` capstone, with the Sense capstone `Eclipse of Resolve` staying alignment-neutral.
- The 2026-08-13 feedback pass restores `Force Burst` as one Force-32 Light area-damage power instead of an additive three-rank line. The balance correction sets it to 44 base damage and 4 SP so the earlier, rider-free power stays below `Force Judgment III`; `Force Judgment I-III` use a 2/2/3 curve. The combined Light damage budget remains 35 SP. Force and Devices remain equal at 240 total SP, 68 perk rows, and 54 `Combat` rows.
