...
- Current behavior:
  ...
  - Reclaims DM-built lightsabers/saberstaffs (any Lightsaber/Saberstaff base item outside the craftable training saber lines and workbench-built sabers) and refunds one Kyber Token per saber. The live login sweep (`_15` + `LegacySaberMigration.MigratePlayer`) grants tokens directly to the player with a message pointing at the Lightsaber Workbenches; stored surfaces (`StoredItemDataMigration` + `LegacySaberMigration.MigrateStoredObject`) swap root-level sabers to tokens in place (market listings are also delisted) and replace nested/droid-held sabers with tokens in their containers. DM creature equipment is intentionally left untouched. Using a Kyber Token item converts it into one `CurrencyType.KyberToken`, and the Lightsaber Workbench placeables (Sith Academy, Jedi Enclave, Dathomir Hidden Cave) consume one of that currency per constructed saber.
