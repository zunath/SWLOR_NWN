...
- Current behavior:
  - Moves active `nar_great_arkanian_dragon` and `smuggler_favor` state-2 records to state 3 before login reconciliation, including outstanding or partial proof counters left by shared corpse loot. Legacy `alchemized_frog` state-2 records without a `frogguts` counter receive the same repair. Their completed boss kills remain credited and they can claim rewards from their givers without farming new drops. Completed quests, earlier/later states, and frog records with an initialized proof counter are unchanged; retries do not grant items or rewards. New progression after server migration 22 continues to require the proof granted to each credited participant.
  ...
  - Reserves droid stat subtype IDs 12–15 for saved legacy grouped weapon skills. Fire/Poison/Electrical/Ice resistance uses 21–24, preventing repeat migrations from converting new resistance bonuses into weapon ranks. Deploy the matching `iprp_droidstat.2da` HAK with the server enum.
  - Normalizes stored droid instructions from current perk definitions: unsupported instructions are removed, retained ranks are clamped, and duplicate ranks, tier limits, and AI slot budgets are enforced on the active list. Valid learned instructions remain available for reprogramming, including ranks deactivated by a tier or slot limit. Controller instruction properties are synchronized with the saved active list. Runtime spawning and the AI editor consume the same policy; JSON is the authoritative active loadout.
