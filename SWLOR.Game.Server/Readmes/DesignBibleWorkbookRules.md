...
- For `SWLOR Design Bible - Combat Upgrade.xlsx`, `tools/UpdateCombatUpgradeAudit.ps1 -RefreshLocalBible` applies `tools/FormatCombatUpgradeBibleWorkbook.ps1` before regenerating audit data. Keep every tab aligned with `tools/CombatUpgradeBibleWorkbookLayout.json`, keep perk-table tabs on the compact layout with narrow `Style`/`SP Price`/metadata columns and a wide `Description` column, keep `Notes` columns at least `45` width units wide, keep `Beast Calcs` condensed enough that columns `A:AE` fit in a normal Sheets viewport, and do not keep fixed row-height bloat that prevents Google Sheets from auto-fitting visible text.
