...
The Design Bible's `Equipment - Droids` instruction table lists all 293 current discs with their controller tiers, concrete skills, and AI slot costs. `Engineering Recipes` includes all 293 recipes, and the ten weapon tabs record per-rank instruction costs and tiers. `System Changes & Migrations` documents eligibility, programming, stance selection, Steel Shoulder targeting, and HP-cost safeguards. `DroidInstructionBibleTables_MatchCurrentDefinitions` checks these catalogue, recipe, and weapon-note entries against the current item, perk, and recipe definitions.
