...
The companion profile already registers every ability and selects granted feats using their existing targeting and activation requirements, including aimed lines and queued attacks. Default self-buff scoring now checks the ability's declared effect types so it does not recast and switch off an active stance. This uses shared ability metadata rather than a list of droid-only exceptions.
...
Validation: after updating to the current master branches, the focused droid, migration, AI, targeting, weapon, and economy run passes 175 tests; the weapon generator passes 25 tests. The build succeeds with eight existing nullable warnings in the NWN API and GuiWidget. Native in-game deployment and combat were not run as part of this review.
