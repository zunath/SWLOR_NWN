...
Steel Shoulder declares an ally-only AI target selector in its ability definition, and regeneration preserves it. The shared ally selector excludes self even when the party list includes the droid, so a low-health droid does not prevent it from guarding another eligible party member.
...
Validation: after updating to the current master branches, the initial focused droid, migration, AI, targeting, weapon, and economy run passed 175 tests. The latest PR follow-up passes 193 focused AI, droid instruction, weapon, stance, and status-effect tests, including competing stances, expiration, ordinary buffs, and Steel Shoulder ally selection. The weapon generator passes 26 tests. The build succeeds with eight existing nullable warnings in the NWN API and GuiWidget. Native in-game deployment and combat were not run as part of this review.
