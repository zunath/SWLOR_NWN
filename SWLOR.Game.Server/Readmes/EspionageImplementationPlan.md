...
- Slicing is a turn-based NUI circuit puzzle with no timers or heartbeat work. Players rotate circuit tiles or swap adjacent tiles to connect the fixed entry node to the fixed core before exhausting trace. Every board is generated from a deterministic, guaranteed-solvable route and is rejected if it begins solved or can be solved by one ordinary rotation or swap.
