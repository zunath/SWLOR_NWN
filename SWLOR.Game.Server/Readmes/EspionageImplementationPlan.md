...
- **Still deferred**: Alertness perk registration + Bible Dev Status flips (must land together with the merge into `feature/combat-upgrade` to avoid a workbook binary conflict; `AlertnessTrait` feat 1500, icon, and TLK are ready), Ghost Protocol's Exposed-on-back-attack rider, hostile-action stealth break polish, Razor/Shock Trap ability definitions (Phase 6), poisons (Phase 5), traps (Phase 6), slicing/lockboxes (Phase 7), module crafting bench + repack.
