...
- **Still deferred**: Espionage-tab Dev Status flips (rows stay Design until playtest numbers settle), NPC trap spawns, the toolset placeable-palette entry for the workbench and trap placeable (palette file has a non-UTF8 byte), and module repack + hak rebuild on deploy.
