...
- **Still deferred**: Espionage-tab Dev Status flips (rows stay Design until playtest numbers settle), crafted trap kits + trap detect/disarm interactions (`TrapDisarm` consumption), NPC trap spawns, the toolset placeable-palette entry for the workbench (palette file has a non-UTF8 byte), and module repack + hak rebuild on deploy.
