...
- **Still deferred**: Alertness perk registration + Espionage-tab Dev Status flips (land with the merge into `feature/combat-upgrade` to avoid workbook binary conflicts — re-apply the worktree Bible edits on top of the target branch workbook at merge time; `AlertnessTrait` feat 1500, icon, and TLK are ready), Ghost Protocol's Exposed-on-back-attack rider, hostile-action stealth break polish, crafted trap kits + trap detect/disarm interactions (`TrapDisarm` consumption), NPC trap spawns, stealth-tick XP, the toolset placeable-palette entry for the workbench (palette file has a non-UTF8 byte), and module repack + hak rebuild on deploy.
