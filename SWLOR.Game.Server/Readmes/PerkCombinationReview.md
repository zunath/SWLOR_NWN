...
The [tracker checklist](PerkRetestChecklist.md) identifies **331 perk/rank rows for verification: 52 direct changes and 279 combination regressions**. The [CSV](PerkRetestChecklist.csv) includes the current tracker status, reason, full scenario and a direct link for every row. These are change-specific testing requirements. At the user's request, the 26 directly changed rows previously marked Pass have been changed to Retest in the live tracker, with every other cell preserved. Subsequent review fixes add direct checks for Creeping Terror and triggered damage: 18 direct rows still say Pass and three Deflecting Return ranks say Not Tested.
...
| Split and periodic damage used inconsistent paths | Converted Force damage could lose the earlier target reduction's budget information. Some DoT paths omitted it, and Bleed skipped the common final mitigation stage. | Physical and converted Force portions carry the same budget information. Converted damage does not apply outgoing type modifiers twice. Bleed, Fragmentation, Force DoTs and Creeping Terror field pulses pass through the appropriate common mitigation stages. A corpus test checks every physical/Force periodic-damage caller so fields cannot silently drop the earlier reduction budget. |
| Payload Pouch did not implement its description | It added single-target auto-attack damage without the thrown-weapon restriction. Its proc fields merged with Savage Reflexes: two 15% effects could become a 30% proc of their combined damage. | Independent rolls. Pouch is a 15% thrown auto-attack proc for 8 Physical DMG on the original target and up to four other hostile targets within 3m. The original target is excluded before selecting secondary targets. Splash cannot trigger on-hit effects, and its damage is deferred until the native attack returns to preserve combat attribution. |
| Explicit zero-damage abilities hit for a minimum amount | The shared percentage floor converted Flash's authored -100% adjustment into a small weapon-damage hit. | Authored zero-damage ability adjustments return zero before outgoing modifiers, flat bonuses, target adjustments and damage floors. Flash retains its status and bonus Enmity without dealing damage. |
...
| Triggered hits skipped typed target defenses | Splash, retaliation and reflected hits omitted Physical/Force damage-taken adjustments. | New triggered hits apply the appropriate typed target adjustment once and carry it into the shared 85% target/generic budget. Converted portions retain their original adjustment, including an explicit zero, without applying a second target modifier. |
| Secondary thrown-ability riders were missing or spent their limits incorrectly | Ricochet Toss granted only a skill selector. Cluster Pouch could select the original target as its one additional enemy and trigger independently for every area victim. | Ricochet Toss grants its 25% chance, 12 damage, 5m radius and one-secondary-target payload. Both riders exclude the original target before limiting candidates and retain independent once-per-cast budgets across targets and recurring impacts. |
...
| Duration bonuses exceeded the hard-control budget | Bonuses could extend an authored 30-second control beyond 30 seconds. | The final combat duration is capped at 30 seconds after outgoing bonuses and resistance. Self-applied system penalties, including the authored 60-second subdual knockdown, are exempt. Soft debuffs retain their duration bonuses. Resistance feedback compares the final constrained durations, including any earlier expiration inherited by control conversion. |
...
**Latest completed full suite: 2,738 passed, zero failed, zero skipped**, including the triggered-damage review fixes. Those changes also passed **208 focused tests**, including typed mitigation, conversion, self-applied subdual duration, ricochet conditions, target selection and independent per-cast rider limits. Builds succeeded with existing nullable-reference/annotation warnings and no errors, and skipped the Windows post-build deploy as required. Ricochet Toss's generator output was checked against its complete authored payload. The final review also added four regressions for explicit zero-damage ability adjustments. Final PR validation records the full suite after that correction.

```powershell
dotnet build SWLOR.Game.Server.Tests/SWLOR.Game.Server.Tests.csproj -p:RunPostBuildEvent=Never --no-restore
dotnet test SWLOR.Game.Server.Tests/SWLOR.Game.Server.Tests.csproj --no-build --no-restore --results-directory TestResults/BibleReview --logger 'trx;LogFileName=pr-triggered-final-full.trx'
```

The completed runs are `TestResults/BibleReview/pr-final-head-full.trx` and `TestResults/BibleReview/pr-triggered-riders-focused.trx` in this workspace. Parent and TLK submodule whitespace checks also passed. The PR records subsequent full validation and review results.
