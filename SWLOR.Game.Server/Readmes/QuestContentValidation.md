...
Pending server migration 22 moves active dragon and sandworm quests already at collection state 2 directly to the reward turn-in state, including outstanding or partial counters left by shared corpse loot. Legacy frog quests at state 2 without a proof counter receive the same repair. It preserves kill credit without awarding items or rewards, leaves earlier/later and completed records alone, and is safe to retry. New quest progression still requires each participant's automatically granted proof. The legacy importer also rejects the retired First Rites `next_state_1` script rather than creating a broken quest action.
