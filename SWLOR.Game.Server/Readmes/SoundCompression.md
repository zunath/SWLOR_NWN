...
Its `--apply` switch and exclusion manifest are required to replace audio; it stages and validates encoded payloads first, preserves resource names, and refuses to overwrite an existing provenance manifest. The command above uses the encoder installed on the authoring machine; pass the path to your FFmpeg build with `libmp3lame` support. Run the current-corpus loop check against the resulting manifest:
