...
Robe 236 additionally uses a complete animation skeleton generated by
`SWLOR_Haks/tools/RobeAnimations.py`. The compiler only matches animation part
numbers against the immediate supermodel's hierarchy. Inheriting through the
original coat omitted the body's neck, hands, pelvis, and leg links; the coat's
helper IDs also overlapped the cloak's IDs. The shared animation models combine
the coat's authored tracks with missing body tracks and remap both against a
complete skeleton. Body clip duration and events are retained, with garment
