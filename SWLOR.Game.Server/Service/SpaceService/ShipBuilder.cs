...
namespace SWLOR.Game.Server.Service.SpaceService
{
    public class ShipBuilder
    {
        ...
        public ShipBuilder RequirePerk(PerkType perkType, int requiredLevel)
        {
            if (requiredLevel < 0)
            {
                _logger.Write<ErrorLogGroup>($"Failed to add required perk to {_activeShip.Name} because requiredLevel cannot be less than zero.");
                return this;
            }
            if (requiredLevel > 100)
            {
                _logger.Write<ErrorLogGroup>($"Failed to add required perk to {_activeShip.Name} because requiredLevel cannot be greater than 100.");
                return this;
            }
