...
{
    [TestFixture]
    public class OnPartyKickAfterTests
    {
        [Test]
        public void Constructor_ShouldCreateInstance()
        {
            // Act
            var eventInstance = new OnPartyKickAfter();

            // Assert
            Assert.That(eventInstance, Is.Not.Null);
            Assert.That(eventInstance.Script, Is.EqualTo("pty_kick_aft"));
        }
        ...
        {
            // Arrange
            var event1 = new OnPartyKickAfter();
            var event2 = new OnPartyKickAfter();
