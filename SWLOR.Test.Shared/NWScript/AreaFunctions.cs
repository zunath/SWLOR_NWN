...
namespace SWLOR.NWN.API.NWScript
{
    public partial class NWScriptServiceMock
    {
        ...
        private class AmbientSoundData
        {
            public bool IsPlaying { get; set; } = false;
            public int DayTrack { get; set; } = 0;
            public int NightTrack { get; set; } = 0;
        ...
        public Vector3 GetPosition(uint oTarget) => _objectPositions.GetValueOrDefault(oTarget, Vector3.Zero);
        ...
        public Vector3 GetAreaLightDirection(AreaLightDirectionType nLightType, uint oArea = OBJECT_INVALID) => 
            GetOrCreateAreaData(oArea).LightDirections.GetValueOrDefault(nLightType, Vector3.Zero);
        ...
        public uint GetFirstInPersistentObject(uint oPersistentObject = OBJECT_INVALID, ObjectType nObjectFilter = ObjectType.All) 
        {
            _currentPersistentObjectIterator = OBJECT_INVALID;
            return _currentPersistentObjectIterator;
        }

        public uint GetNextInPersistentObject(uint oPersistentObject = OBJECT_INVALID, ObjectType nObjectFilter = ObjectType.All) 
        {
            _currentPersistentObjectIterator = OBJECT_INVALID;
            return _currentPersistentObjectIterator;
        }
