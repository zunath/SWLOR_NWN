...
namespace SWLOR.NWN.API.NWScript
{
    public partial class NWScriptServiceMock
    {
        ...
        public struct ItemProperty
        {
            ...
            public static ItemProperty Invalid => new ItemProperty { Id = 0 };
        }
        
        public enum ItemPropertyDamageResistanceType
        {
            Invalid = 0,
            Physical = 1,
            Magical = 2
        ...
        public ItemProperty GetFirstItemProperty(uint oItem) { var properties = _itemProperties.GetValueOrDefault(oItem, new List<ItemProperty>()); return properties.Count > 0 ? properties[0] : ItemProperty.Invalid; }
        public ItemProperty GetNextItemProperty(uint oItem) => ItemProperty.Invalid;
        ...
        public ItemProperty ItemPropertyDamageResistance(ItemPropertyDamageType nDamageType, ItemPropertyDamageResistanceType nResistance) => CreateItemProperty(ItemPropertyType.DamageResistance);
        ...
        public int GetItemPropertyUsesPerDayRemaining(uint oItem, IntPtr ip) => _propertyUsesPerDay.GetValueOrDefault(oItem, new Dictionary<ItemProperty, int>()).GetValueOrDefault(new ItemProperty(), 0);
        public void SetItemPropertyUsesPerDayRemaining(uint oItem, IntPtr ip, int nUsesPerDay) { if (!_propertyUsesPerDay.ContainsKey(oItem)) _propertyUsesPerDay[oItem] = new Dictionary<ItemProperty, int>(); _propertyUsesPerDay[oItem][new ItemProperty()] = nUsesPerDay; }
        public ItemProperty ItemPropertyCustom(ItemPropertyType nType, int nSubType = -1, int nCostTableValue = -1, int nParam1Value = -1) { var property = new ItemProperty(); _propertyTypes[property] = nType; _propertySubTypes[property] = nSubType; _propertyCostTableValues[property] = nCostTableValue; _propertyParam1Values[property] = nParam1Value; return property; }

        private ItemProperty CreateItemProperty(ItemPropertyType type) { var property = new ItemProperty(); _propertyTypes[property] = type; return property; }
    }
}
