...
namespace SWLOR.Test.Shared.NWScriptMocks
{
    public partial class NWScriptServiceMock
    {
        ...
        // Additional creature methods from INWScriptService
        public string GetDeity(uint oCreature) => GetOrCreateCreatureData(oCreature).Deity;
        public string GetSubRace(uint oTarget) => GetOrCreateCreatureData(oTarget).SubRace;
        public int GetFortitudeSavingThrow(uint oTarget) => 10; // Mock implementation
        public int GetWillSavingThrow(uint oTarget) => 10; // Mock implementation
        public int GetReflexSavingThrow(uint oTarget) => 10; // Mock implementation
        public float GetChallengeRating(uint oCreature) => 1.0f; // Mock implementation
        public int GetAge(uint oCreature) => 25; // Mock implementation
        public int GetMovementRate(uint oCreature) => 30; // Mock implementation
        public bool GetPlotFlag(uint oTarget = OBJECT_INVALID) => false; // Mock implementation
        public void SetPlotFlag(uint oTarget, bool nPlotFlag) { } // Mock implementation
        public int GetGold(uint oTarget = OBJECT_INVALID) => _creatureGold.GetValueOrDefault(oTarget, 0);
        public uint GetNearestCreatureToLocation(CreatureType nFirstCriteriaType, bool nFirstCriteriaValue, Location lLocation,
            int nNth = 1, int nSecondCriteriaType = -1, int nSecondCriteriaValue = -1, int nThirdCriteriaType = -1,
            int nThirdCriteriaValue = -1) => OBJECT_INVALID; // Mock implementation
        public uint GetNearestCreature(CreatureType nFirstCriteriaType, int nFirstCriteriaValue,
            uint oTarget = OBJECT_INVALID, int nNth = 1, int nSecondCriteriaType = -1,
            int nSecondCriteriaValue = -1, int nThirdCriteriaType = -1, int nThirdCriteriaValue = -1) => OBJECT_INVALID; // Mock implementation
        public int GetReflexAdjustedDamage(int nDamage, uint oTarget, int nDC, SavingThrowType nSaveType = SavingThrowType.All,
            uint oSaveVersus = OBJECT_INVALID) => nDamage; // Mock implementation
        public int LevelUpHenchman(uint oCreature, ClassType nClass = ClassType.Invalid, bool bReadyAllSpells = false,
            PackageType nPackage = PackageType.Invalid) => 1; // Mock implementation

        // Helper methods for testing
        public Dictionary<uint, CreatureData> GetCreatureData() => new Dictionary<uint, CreatureData>(_creatureData);
        public void ClearCreatureData() => _creatureData.Clear();
        public void SetCreatureGold(uint oCreature, int amount) => _creatureGold[oCreature] = amount;
        public void SetCreatureXP(uint oCreature, int amount) => _creatureXP[oCreature] = amount;
        public void AddCreatureFeat(uint oCreature, FeatType feat) 
        {
            if (!_creatureFeats.ContainsKey(oCreature))
                _creatureFeats[oCreature] = new HashSet<FeatType>();
            _creatureFeats[oCreature].Add(feat);
        }
        public void AddCreatureSpell(uint oCreature, SpellType spell) 
        {
            if (!_creatureSpells.ContainsKey(oCreature))
                _creatureSpells[oCreature] = new HashSet<SpellType>();
            _creatureSpells[oCreature].Add(spell);
        }
        public void SetCreatureSkill(uint oCreature, NWNSkillType skill, int rank) 
        {
            if (!_creatureSkills.ContainsKey(oCreature))
                _creatureSkills[oCreature] = new Dictionary<NWNSkillType, int>();
            _creatureSkills[oCreature][skill] = rank;
        }
        public void SetCreatureAbility(uint oCreature, AbilityType ability, int value) 
        {
            if (!_creatureAbilities.ContainsKey(oCreature))
                _creatureAbilities[oCreature] = new Dictionary<AbilityType, int>();
            _creatureAbilities[oCreature][ability] = value;
        }
        public void SetCreatureLevel(uint oCreature, ClassType classType, int level) 
        {
            if (!_creatureLevels.ContainsKey(oCreature))
                _creatureLevels[oCreature] = new Dictionary<ClassType, int>();
            _creatureLevels[oCreature][classType] = level;
        }
        public void SetCreatureImmunity(uint oCreature, ImmunityType immunity, bool value) 
        {
            if (!_creatureImmunities.ContainsKey(oCreature))
                _creatureImmunities[oCreature] = new Dictionary<ImmunityType, bool>();
            _creatureImmunities[oCreature][immunity] = value;
        }
