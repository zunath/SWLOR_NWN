...
public static class AnimationInstall
{
        ...
        int outputBudget = MaximumOutputBytes, string? sourceProjectPath = null, int bankBudget = AnimationProject.MaximumFileBytes)
    {
        ...
        registrations.Add(new(project.Name, animationName, project.Duration, targetNames, relativeProjectPath));
        ...
        var projectBytes = Encoding.UTF8.GetBytes(serializedProject + "\n");
        // Preserve identical source bytes, including line endings, so installing a generated
        // project does not invalidate its provenance hash or produce a formatting-only edit.
