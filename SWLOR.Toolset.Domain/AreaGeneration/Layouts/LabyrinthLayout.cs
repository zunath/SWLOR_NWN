...
{
    /// <summary>
    /// Classic labyrinth: a near-perfect maze of long, winding, 1-corridor-wide passages with only a
    /// handful of small chambers carved at corridor junctions. Distinct from <see cref="WarrenLayout"/>
    /// in three ways: (1) maze carving is directionally biased toward continuing straight, producing
    /// long serpentine runs instead of Warren's uniformly-random branching; (2) the loop-removal pass
    /// is driven by the same LoopFactor knob but the labyrinth profile defaults it very low, so most of
    /// the spanning tree survives and dead ends/backtracking dominate; (3) chambers are few (profile
    /// default ~3-4) and preferentially sited at maze junctions (cells where 3+ passages meet) rather
    /// than sampled from every cell, so they read as clearings carved out of the maze rather than
    /// Warren's frequent rooms.
    /// </summary>
    internal static class LabyrinthLayout
    {
        /// <summary>Probability the carver continues straight through a junction cell when a straight,
        /// unvisited neighbor is available. Biases the maze toward long runs with occasional serpentine
        /// turns instead of Warren's direction-agnostic branching.</summary>
        private const double StraightBias = 0.7;

        /// <summary>Chambers are capped smaller than Warren's (5) so they read as tight clearings.</summary>
        private const int MaxChamberCornerSize = 4;
        ...
        internal static MacroLayout Generate(MacroLayoutParameters parameters, System.Random random)
        {
            ...
            layout.Rooms = CarveChambers(corners, parameters, passages, cellsX, cellsY, pitch, corridorWidth, random);
            return layout;
        }
        ...
        /// <summary>
        /// Directionally-biased recursive backtracker (iterative, stack-based) growing-tree maze:
        /// at each step, if the current cell was entered along some direction and the cell straight
        /// ahead in that direction is unvisited, continue straight with probability
        /// <see cref="StraightBias"/>; otherwise pick uniformly among shuffled unvisited neighbors.
        /// This produces long straight runs punctuated by serpentine turns, rather than Warren's
        /// direction-agnostic branching.
        /// </summary>
        ...
        {
            var arrivalDir = new Dictionary<(int X, int Y), (int Dx, int Dy)>();
            var stack = new Stack<(int X, int Y)>();
            var start = (X: random.Next(cellsX), Y: random.Next(cellsY));
            visited[start.X, start.Y] = true;
            arrivalDir[start] = (0, 0);
            stack.Push(start);
            ...
            while (stack.Count > 0)
            {
                ...
                var lastDir = arrivalDir[current];
                var straightTarget = (current.X + lastDir.Dx, current.Y + lastDir.Dy);

                (int X, int Y) next;
                if (lastDir != (0, 0) && neighbors.Contains(straightTarget) && random.NextDouble() < StraightBias)
                    next = straightTarget;
                else
                    next = neighbors[0];

                visited[next.X, next.Y] = true;
                arrivalDir[next] = (next.X - current.X, next.Y - current.Y);
        ...
        /// <summary>
        /// Carves a small number of chambers, preferring cells where 3+ passages meet (junctions) so
        /// chambers read as clearings carved out of the maze rather than Warren's frequent, arbitrarily
        /// placed rooms. Falls back to sampling every cell when the maze has too few junctions (e.g.
        /// a very narrow grid).
        /// </summary>
        private static List<LayoutRoom> CarveChambers(
            CornerTerrainGrid corners, MacroLayoutParameters parameters,
            HashSet<((int X, int Y) A, (int X, int Y) B)> passages,
            int cellsX, int cellsY, int pitch, int corridorWidth, System.Random random)
        {
            ...
            if (allCells.Count < 2)
            {
                throw new InvalidOperationException(
                    $"Labyrinth maze only produced {allCells.Count} cell(s) in a {parameters.Width}x{parameters.Height} " +
                    "area; at least 2 are required for chambers.");
            }

            var degree = allCells.ToDictionary(c => c, _ => 0);
            foreach (var (a, b) in passages)
            {
                degree[a]++;
                degree[b]++;
            ...
            var junctionCells = allCells.Where(c => degree[c] >= 3).ToList();
            var candidates = junctionCells.Count >= chamberCount ? junctionCells : allCells;

            var chosenCells = LayoutCornerUtils.FarthestPointSample(candidates, chamberCount, random);

            var minSize = Math.Max(2, parameters.MinRoomCornerSize);
            var maxSize = Math.Max(minSize, Math.Min(parameters.MaxRoomCornerSize, MaxChamberCornerSize));
