...
public class MeleeChoreographyTests
{
    ...
    [TestCase("Lightsaber", "lightsaber")]
    [TestCase("Saberstaff", "saberstaff")]
    [TestCase("Staff", "staff")]
    [TestCase("Spear", "spear")]
    [TestCase("Twin Blade", "twin-blade")]
    [TestCase("Vibroknife", "vibroknife")]
    public void WeaponCategoriesHaveCompleteDistinctMotionsAndNativeGrips(string category, string folder)
    {
        ...
        for (var j = i + 1; j < ids.Length; j++)
        {
            ...
            // Timing changes can move an otherwise identical strike along the normalized timeline.
            // Compare the ordered native action phases independently of beat timing as well.
            static string Shape(Choreography recipe) => string.Join("|", recipe.Beats
                .Select(beat => beat.SourceAnimation + ":" + (beat.SourceTime < .2f ? "entry" : beat.SourceTime < .4f ? "windup" : beat.SourceTime < .7f ? "action" : "recovery")
                    + ":" + JsonSerializer.Serialize(new { beat.LeftHand, beat.RightHand, beat.TorsoDegrees }))
                .Aggregate(new List<string>(), (list, phase) => { if (list.Count == 0 || list[^1] != phase) list.Add(phase); return list; }));
            Shape(recipes.Single(recipe => recipe.Id == ids[i])).Should().NotBe(
                Shape(recipes.Single(recipe => recipe.Id == ids[j])),
                ids[i] + " and " + ids[j] + " must not reuse the same ordered action with only a timing change");
        }
    }
