...
{
    /// <summary>
    /// Puts the left rail's tool tabs above its panel instead of below it.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// tabs belong where the eye already is - at the top of the rail, next to the panel title.
    /// Palette/Script Reference and the Output group are consulted rather than lived in, and their
    /// tabs are better off out of the way at the bottom where Dock puts them. A style on
    /// <c>ToolControl</c> would have moved all three.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// </remarks>
    public static class ToolTabStripPlacement
    {
        /// <summary>The docks whose tab strip belongs above the panel.</summary>
        private static readonly HashSet<string> TopStripDockIds =
        ...
        private static void OnTemplateApplied(ToolControl control, TemplateAppliedEventArgs e)
        {
            ...
            // dock it is decides where the strip goes. Unsubscribed first so a re-templated control
        ...
        private static void Apply(ToolControl control)
        {
            ...
            // Bottom is Dock's own default, and setting it back matters: a panel dragged out of the
            // left rail and dropped elsewhere would otherwise keep the top strip it had there.
            DockPanel.SetDock(strip, WantsTopStrip(control) ? AvaloniaDock.Top : AvaloniaDock.Bottom);
        }

        private static bool WantsTopStrip(ToolControl control) =>
            control.DataContext is IDock { Id: { } id } && TopStripDockIds.Contains(id);
    }
}
