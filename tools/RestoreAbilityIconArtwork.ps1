...
$referenceDirectory = if (Test-Path -LiteralPath $ReferenceArtworkPath) {
    (Resolve-Path -LiteralPath $ReferenceArtworkPath).Path
}
else {
    $ReferenceArtworkPath
}
$sheetDirectory = if (Test-Path -LiteralPath $GeneratedSheetPath) {
    (Resolve-Path -LiteralPath $GeneratedSheetPath).Path
}
else {
    $null
}
...
$tiles = if ($null -ne $sheetDirectory) {
    [IconArtworkRestore]::BuildTileLibrary($sheetDirectory, $IconSize)
}
else {
    $null
}
